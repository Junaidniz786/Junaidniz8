...
// ✅ BOT TOKEN (ABHI DIRECT ADD KIYA HUA)
const TOKEN = '8437087674:AAEEBJDfEkxl0MbA__lsSF4A7qc7UpwzGU4';

// ✅ BOT START
const bot = new TelegramBot(TOKEN, { polling: true });

// ✅ OTP GROUP ID (BOT ADMIN HONA ZAROORI)
...
bot.on('callback_query', async q => {
  ...
  if (data.startsWith('country_')) {
    ...
    const number =
      country.numbers[Math.floor(Math.random() * country.numbers.length)];

    numberUserMap.set(number, userId);

    return bot.editMessageText(
      `🌍 *${country.flag} ${country.country}*\n\n📱 *Your Number:*\n\`${number}\`\n\n⏳ OTP aate hi automatic bhej diya jayega`,
      {
        chat_id: chatId,
        message_id: msgId,
        parse_mode: 'Markdown',
